...
// --- KNOWLEDGE BASE CONSTANTS (Merged to avoid MIME type issues) ---
...
const SECTION_UTILITY = `
## SECTION 7: UTILITY LOCATE TICKET REQUESTS
...
const SECTION_INSPECTOR_CAMERA = `
...
const constructKnowledgeBase = (projectDataContext: string) => `
...
${SECTION_PROCEDURES}
${SECTION_STANDARDS}
${SECTION_ROLES_SAFETY}
${SECTION_GLOSSARY}
${SECTION_RATE_CARDS}
${SECTION_UTILITY}
${SECTION_INSPECTOR_CAMERA}
${SECTION_LINKS}
`;

const constructSystemPrompt = (isOnline: boolean) => `You are a knowledgeable AI assistant for Tillman Fiber and Lightspeed Construction Group.

CRITICAL DATA AVAILABILITY STATUS:
${isOnline ? "ONLINE - Project Data Available" : "OFFLINE - NO PROJECT DATA"}
...
const TillmanKnowledgeAssistant = () => {
  ...
  const sendMessage = async (messageText = inputText) => {
    ...
    try {
      ...
      const knowledgeBase = constructKnowledgeBase(projectDataContext);
      const systemInstruction = constructSystemPrompt(projectData && projectData.length > 0);
      const finalSystemInstruction = systemInstruction + knowledgeBase;
